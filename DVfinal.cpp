...
int handle_intersectingpath(vector<int> &diverge, vector<int> path, map<int, pair<int, int>> &present, map<pair<int, int>, int> &w, int outdegree[], pathinfo *p, int safepathno, int b)
{
    ...
    // cout << l_ << " " << r_ << endl;
    // cout << present[path[l]].second << endl;
    // cout<<"yo="<<path[l+1]<<endl;
...
int main()
{
    ...
    while (t--)
    {
        ...
        for (int fileno = 0; fileno < 100; fileno++)
        {
            ...
            for (int i = 0; i < path.size() - 1; i++)
            {
                ...
                w[{path[i], path[i + 1]}] += b;
                outdegree[path[i]] += b;
                // cout << present[path[i]].first << "space" << present[path[i]].second << endl;
            }
            ...
            {
                for (int i = 0; i < pathno[t].safety.size(); ++i)
                {
                    bool flag = check_redundancy(&pathno[t], i);
                    if (flag == 1)
                        i--;
                }
            }
    ...
    fstream f;
    f.open("dyn6.csv", ios::out | ios::app);
    for (auto i : master)
    {
        for (auto k : i)
        {
